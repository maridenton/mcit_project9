...
object jdbcMainObject extends hClient with App {
  ...
  stmt.execute("drop table IF EXISTS fall2019_marinda.ext_station_information")
  stmt.execute("drop table IF EXISTS fall2019_marinda.ext_system_information")
  stmt.execute("drop table IF EXISTS fall2019_marinda.enriched_station_information")

  stmt.execute(
    """create external table fall2019_marinda.ext_station_information(
      |    station_id int,
      |    external_id string,
      |    name string,
      |    short_name int,
  ...
  stmt.execute(
    """create external table fall2019_marinda.ext_system_information(
      |   system_id string,
      |   timezone string
      |)
      |row format delimited
  ...
  stmt.execute(
    """create table fall2019_marinda.enriched_station_information(
      |   system_id string,
      |   timezone string,
      |   station_id int,
      |   name string,
  ...
  stmt.execute(
    """insert into table fall2019_marinda.enriched_station_information
      |SELECT
      |    sy.system_id,
      |    sy.timezone,
      |    st.station_id,
      ...
      |    st.lon,
      |    st.capacity
      |FROM fall2019_marinda.ext_station_information as st
      |    cross join fall2019_marinda.ext_system_information as sy""".stripMargin)
